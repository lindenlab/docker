...
func TestMapPorts(t *testing.T) {
	...
	if host, err := pm.Map(srcAddr1, dstIp1, 80, 80); err != nil {
	...
	if _, err := pm.Map(srcAddr1, dstIp1, 80, 80); err == nil {
		t.Fatalf("Port is in use - mapping should have failed")
	}

	if _, err := pm.Map(srcAddr2, dstIp1, 80, 80); err == nil {
		t.Fatalf("Port is in use - mapping should have failed")
	}

	if _, err := pm.Map(srcAddr2, dstIp2, 80, 80); err != nil {
		t.Fatalf("Failed to allocate port: %s", err)
	}
...
func TestMapAllPortsSingleInterface(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		for i := start; i < end; i++ {
			if host, err = pm.Map(srcAddr1, dstIp1, 0, 0); err != nil {
				t.Fatal(err)
			}
		...
		if _, err := pm.Map(srcAddr1, dstIp1, start, start); err == nil {
