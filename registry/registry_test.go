...
func TestNewRepositoryInfo(t *testing.T) {
	...
	config.JobPolicies["pull"] = &JobPolicy {
		ForceQualified: true,
	}
	config.JobPolicies["search"] = &JobPolicy {
	...
	_, err = config.NewRepositoryInfo(IndexServerName()+"/library/ubuntu", "pull")
	if err != nil {
	...
	_, err = config.NewRepositoryInfo(IndexServerName()+"/library/ubuntu", "search")
	if err != nil {
		t.Fatal("Expected no error with qualified repo name: " + IndexServerName() + "/library/ubuntu")
	}
