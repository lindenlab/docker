...
func (s *DockerSuite) TestRunWithPull(c *check.C) {
	...
	out, _, err := dockerCmdWithError("run", "hello-world")
...
func (s *DockerTrustSuite) TestTrustedRunWithPull(c *check.C) {
	...
	runCmd = exec.Command(dockerBinary, "--debug", "run", "--pull", repoName)
	s.trustedCmd(runCmd)
	out, _, err = runCommandWithOutput(runCmd)
	c.Assert(err, check.IsNil, check.Commentf(out))
	...
	runCmd = exec.Command(dockerBinary, "--debug", "run", "--pull=false", repoName)
	s.trustedCmd(runCmd)
	out, _, err = runCommandWithOutput(runCmd)
	c.Assert(err, check.IsNil, check.Commentf(out))
